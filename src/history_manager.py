...
class HistoryManager:
    ...
    Cấu trúc file:
    {
        "123456789": {
            ...
            "searches": [
                {"query": "Doraemon", "timestamp": "2024-01-15 20:30:00", "results_count": 3},
                ...
            ]
        },
        ...
    ...
    def get(self, user_id: int, limit: int = 10) -> list[dict]:
        """Lấy `limit` lượt tìm kiếm gần nhất (mới nhất lên đầu)."""
        uid = str(user_id)
        with self._lock:
            data     = self._read()
    ...
    def get_with_stats(self, user_id: int, limit: int = 10) -> tuple[list[dict], dict]:
        """Đọc file 1 lần, trả về (records, stats) cùng lúc."""
        uid = str(user_id)
        with self._lock:
            data     = self._read()
            searches = data.get(uid, {}).get("searches", [])

        records = list(reversed(searches[-limit:]))

        if not searches:
            stats = {"total": 0, "top_queries": []}
        else:
